...
struct memoryMap
{
    ...
    byte PhotoMSB;   // Reg: 0x01 - Current Joystick Horizontal Position (MSB)
    byte PhotoLSB;   // Reg: 0x02 - Current Joystick Horizontal Position (LSB)
    byte i2cLock;    // Reg: 0x03 - Must be changed to 0x13 before I2C address can be changed.
    byte i2cAddress; // Reg: 0x04 - Set I2C New Address (re-writable). Clears i2cLock.
};
